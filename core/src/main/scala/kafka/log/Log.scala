...
class Log(val dir: File,
  ...
  def deleteFiberhomeOldSegments(orderedList:util.List[(TopicAndPartition,Long)]):Int ={
    if (!config.delete) return 0
    deleteRetenionDiskSizeBreachedSegments(orderedList)
  }
  ...
  /**
    ...
    * @return
    */
  private def deleteRetenionDiskSizeBreachedSegments(orderedList:util.List[(TopicAndPartition,Long)]):Int ={
    val currentDir = new File(dir.getAbsolutePath)
    try{
      val totalSize = currentDir.getTotalSpace
      val freeSize = currentDir.getFreeSpace
      ...
      case e:Exception =>
        error("[fiberhome] deleteRetenionDiskSizeBreachedSegments error : ",e)
    }
    return 0
  }
