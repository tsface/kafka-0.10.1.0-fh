...
class LogManager(val logDirs: Array[File],
  ...
  def cleanupLogs() {
    ...
    //排序
    val orderedLogList : util.List[(TopicAndPartition,Long)]= new util.ArrayList[(TopicAndPartition,Long)]()
    ...
    for(log <- allLogs; if !log.config.compact) {
      debug("Fiberhome garbage collecting '" + log.name + "'")
      total += log.deleteFiberhomeOldSegments(orderedLogList)
    }
